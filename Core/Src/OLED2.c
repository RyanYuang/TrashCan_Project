...
#define OLED2_W_SDA(x) HAL_GPIO_WritePin(GPIOB, GPIO_PIN_12, x)
...
void OLED2_ShowFloat(uint8_t Line, uint8_t Column, float Number, uint8_t Length, uint8_t Decimal_Places)
{
    ...
    Decimal_Part = (uint32_t)((Number - Integer_Part) * OLED2_Pow(10, Decimal_Places));

    // 显示整数部分
    OLED2_ShowNum(Line, Column, Integer_Part, Length);

    // 显示小数点（如果有小数位）
    if (Decimal_Places > 0) {
        OLED2_ShowChar(Line, Column + Length, Decimal_Char);
        // 显示小数部分作为整数（e.g., 0.14 → 14）
        OLED2_ShowNum(Line, Column + Length + 1, Decimal_Part, Decimal_Places);
    }
}
