 ...
 * @param  Length 整数部分长度，范围：1~10
 * @param  Decimal_Places 小数位数，范围：0~6
 * @retval 无
 */
void OLED_ShowFloat(uint8_t Line, uint8_t Column, float Number, uint8_t Length, uint8_t Decimal_Places)
{
    uint32_t Integer_Part;  // 整数部分
    uint32_t Decimal_Part;  // 小数部分
    char Decimal_Char = '.';  // 小数点字符

    if (Number < 0) {
        Number = -Number;  // 暂不支持负数，绝对值处理（可扩展）
    }

    // 分离整数和小数
    Integer_Part = (uint32_t)Number;
    Decimal_Part = (uint32_t)((Number - Integer_Part) * OLED_Pow(10, Decimal_Places));

    // 显示整数部分
    OLED_ShowNum(Line, Column, Integer_Part, Length);

    // 显示小数点（如果有小数位）
    if (Decimal_Places > 0) {
        OLED_ShowChar(Line, Column + Length, Decimal_Char);
        // 显示小数部分作为整数（e.g., 0.14 → 14）
        OLED_ShowNum(Line, Column + Length + 1, Decimal_Part, Decimal_Places);
    }
}
...
{
	// 127 对应字库索引 95（°），因为 95 + 32 = 127
	OLED_ShowChar(Line, Column, 127);
...
void OLED_ShowIcon24x24(uint8_t x, uint8_t y, const uint8_t *bitmap)
{
    ...
    {
        OLED_SetCursor(y + j, x);  // 设置光标到对应页
        for (i = 0; i < 24; i++)   // 24列（24像素宽）
        {
            OLED_WriteData(bitmap[j * 24 + i]);
        }
    }
}
